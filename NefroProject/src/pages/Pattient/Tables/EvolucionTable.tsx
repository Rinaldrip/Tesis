...
interface PatientEvolutionForm {
    fecha: string;
    evolucion: string;
}
...
export default function PatientEvolutionTable({
    data = [],
    loading = false,
    onAddItem,
    onDeleteItem,
    ...
    const [patientEvolutions, setPatientEvolutions] = useState<PatientEvolution[]>(data);
    ...
    const onSubmit = async (formData: PatientEvolutionForm) => {
        if (!formData.fecha || !formData.evolucion.trim()) return;

        setIsSubmitting(true);
        try {
            const evolutionData: Omit<PatientEvolution, 'id'> = {
                fecha: formData.fecha,
                evolucion: formData.evolucion.trim()
            };

            if (onAddItem) {
                await onAddItem(evolutionData);
                if (refetch) {
                    await refetch();
                }
            } else {
                console.warn("No hay función onAddEvolution proporcionada");
            }
    ...
    const handleDeleteRow = async (id: string) => {
        ...
        try {
            if (onDeleteItem) {
                ...
                console.warn("No hay función onDeleteEvolution proporcionada");
            }
    ...
    const applyDateFilter = (evolutions = patientEvolutions) => {
        if (!dateFilter.from && !dateFilter.to) return evolutions;
        return evolutions.filter(evolution => {
            const d = new Date(evolution.fecha);
    ...
    const filteredEvolutions = applyDateFilter();
    const totalPages = Math.ceil(filteredEvolutions.length / ITEMS_PER_PAGE);

    const paginatedEvolutions = useMemo(() => {
        const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
        return filteredEvolutions.slice(startIndex, startIndex + ITEMS_PER_PAGE);
        ...
        return (
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
                <div className="flex items-center justify-center py-12">
                    ...
                    <span className="ml-2 text-gray-600">Cargando evoluciones...</span>
                </div>
            </div>
        );
    }
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-purple-600 to-purple-700 px-6 py-4">
                <div className="flex justify-between items-center">
                    <div>
                        <h2 className="text-xl font-semibold text-white">Evolución del Paciente</h2>
                        <p className="text-purple-100 text-sm mt-1">
                            {patientEvolutions.length} {patientEvolutions.length === 1 ? 'registro' : 'registros'} de evolución
                        </p>
                    </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                                ...
                                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-800">Evolución</th>
                                {(isAdding || isDeleteMode) && (
                        ...
                        <tbody>
                            ...
                            {isAdding && (
                                <tr className="border-b border-gray-100 bg-green-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <textarea
                                            {...register("evolucion", {
                                                required: "La evolución es requerida",
                                                minLength: {
                                                    value: 10,
                                                    message: "La evolución debe tener al menos 10 caracteres"
                                                }
                                            })}
                                            className={`w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-green-500 min-h-[80px] resize-vertical ${errors.evolucion ? "border-red-500" : "border-gray-300"
                                                }`}
                                            placeholder="Describa la evolución del paciente en esta consulta..."
                                        />
                                        {errors.evolucion && (
                                            <p className="text-red-500 text-xs mt-1">{errors.evolucion.message}</p>
                                        )}
                                    </td>
                                    <td className="px-6 py-4 align-top">
                            ...
                            {paginatedEvolutions.map(evolution => (
                                <tr key={evolution.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                    <td className="px-6 py-4 text-sm font-medium text-gray-900 align-top">{formatDate(evolution.fecha)}</td>
                                    ...
                                    {(isAdding || isDeleteMode) && (
                                        <td className="px-6 py-4">
                                            {isDeleteMode ? (
                                                <button
                                                    type="button"
                                                    onClick={() => handleDeleteRow(evolution.id)}
                                                    disabled={isDeleting === evolution.id}
                            ...
                            {paginatedEvolutions.length === 0 && !isAdding && (
                                <tr>
