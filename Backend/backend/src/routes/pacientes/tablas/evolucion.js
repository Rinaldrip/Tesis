...
router.get("/pacientes/evoluciones/:cedula", async (req, res) => {
    try {
        ...
        try {
            const evolucionResult = await client.query(
                'SELECT * FROM evoluciones WHERE cedula_paciente = $1 ORDER BY fecha DESC', 
                [cedula]
            );

            res.json({ 
                success: true,
                evoluciones: evolucionResult.rows
            });
...
router.post("/pacientes/evoluciones", async (req, res) => {
    const client = await pool.connect();
    try {
        ...
        const query = `
            ...
            (cedula_paciente, fecha, evolucion)
            VALUES ($1, $2, $3)
            RETURNING id
        `;
        ...
        const values = [
            ...
            safeValue(testData.evolucion, 'string')
        ];
...
router.delete("/pacientes/evoluciones/eliminate/:id", async (req, res) => {
    const client = await pool.connect();
    ...
    try {
        ...
        const checkQuery = 'SELECT id, cedula_paciente FROM evoluciones WHERE id = $1';
        ...
        if (checkResult.rows.length === 0) {
            ...
            const allIds = await client.query('SELECT id FROM evoluciones ORDER BY id');
        ...
        const deleteQuery = 'DELETE FROM evoluciones WHERE id = $1 RETURNING id, cedula_paciente';
