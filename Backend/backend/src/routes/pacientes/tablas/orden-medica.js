...
router.get("/pacientes/ordenes-medicas/:cedula", async (req, res) => {
    try {
        const { cedula } = req.params;
        const client = await pool.connect();

        try {
            const ordenMedicaResults = await client.query(
                'SELECT * FROM ordenes_medicas WHERE cedula_paciente = $1 ORDER BY fecha DESC', 
                [cedula]
            );

            res.json({ 
                success: true,
                ordenes_medicas: ordenMedicaResults.rows  
            });
...
router.post("/pacientes/ordenes-medicas", async (req, res) => {
    const client = await pool.connect();
    try {
        ...
        const values = [
            ...
            safeValue(testData.orden, 'string')
        ];
        ...
        res.status(201).json({ 
            success: true,
            message: "Orden medica guardado correctamente ✅",
            id: testId,
        ...
        console.error("Error completo al registrar la orden medica:", error);
        res.status(500).json({ 
            error: "Error al registrar la orden medica",
...
router.delete("/pacientes/ordenes-medicas/eliminate/:id", async (req, res) => {
    const client = await pool.connect();
    ...
    try {
        ...
        const tableCheck = await client.query(`
            SELECT EXISTS (
                ...
                WHERE table_name = 'ordenes_medicas'
            );
        `);
        ...
        if (!tableCheck.rows[0].exists) {
            ...
            return res.status(500).json({ 
                ...
                details: "La tabla 'ordenes_medicas' no existe en la base de datos"
            });
        }

        // SEGUNDO: Verificar si el registro existe
        const checkQuery = 'SELECT id, cedula_paciente FROM ordenes_medicas WHERE id = $1';
        ...
        if (checkResult.rows.length === 0) {
            ...
            const allIds = await client.query('SELECT id FROM ordenes_medicas ORDER BY id');
            console.log('📋 IDs existentes en la tabla:', allIds.rows.map(row => row.id));
            
            return res.status(404).json({ 
                success: false,
                error: "Registro no encontrado",
                details: `No existe ningún Orden medica con ID ${testId}`,
                availableIds: allIds.rows.map(row => row.id)
            });
        }

        // TERCERO: Eliminar el registro
        const deleteQuery = 'DELETE FROM ordenes_medicas WHERE id = $1 RETURNING id, cedula_paciente';
